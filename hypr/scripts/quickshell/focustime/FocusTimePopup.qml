...
Item {
   ...
   // -------------------------------------------------------------------------
    ...
    // Must match focus_daemon.py (not $XDG_RUNTIME_DIR — Quickshell may not inherit it).
    readonly property string stateFilePath: Quickshell.env("HOME") + "/.cache/focustime/focustime_state.json"
    ...
    Process {
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                if (window.selectedAppClass === "" && window.isTodaySelected) {
                    statsPoller.command = ["python3", window.scriptsDir + "/get_stats.py", getIsoDate(window.activeDate)];
                    statsPoller.running = true;
                }
            }
        }
