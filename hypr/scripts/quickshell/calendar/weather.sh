...
# Icons for JSON / QML: emoji render with system color emoji fallback (no Nerd Font PUA).
get_icon() {
    case $1 in
        "50d"|"50n") icon='🌫️'; quote="Mist" ;;
        "01d") icon='☀️'; quote="Sunny" ;;
        "01n") icon='🌙'; quote="Clear" ;;
        "02d"|"02n"|"03d"|"03n"|"04d"|"04n") icon='☁️'; quote="Cloudy" ;;
        "09d"|"09n"|"10d"|"10n") icon='🌧️'; quote="Rainy" ;;
        "11d"|"11n") icon='⛈️'; quote="Storm" ;;
        "13d"|"13n") icon='❄️'; quote="Snow" ;;
        *) icon='☁️'; quote="Unknown" ;;
    esac
    printf '%s|%s\n' "$icon" "$quote"
}
...
write_dummy_data() {
    ...
    for i in {0..4}; do
        ...
        final_json="${final_json} {
            ...
            \"desc\": \"No API Key\",
            \"hourly\": [
                {\"time\": \"00:00\", \"temp\": \"18.0\", \"icon\": \"🌙\", \"hex\": \"#cba6f7\", \"owm\": \"01n\"},
                {\"time\": \"03:00\", \"temp\": \"17.0\", \"icon\": \"☁️\", \"hex\": \"#bac2de\", \"owm\": \"03d\"},
                {\"time\": \"06:00\", \"temp\": \"19.0\", \"icon\": \"☁️\", \"hex\": \"#bac2de\", \"owm\": \"04d\"},
                {\"time\": \"09:00\", \"temp\": \"21.0\", \"icon\": \"☀️\", \"hex\": \"#f9e2af\", \"owm\": \"02d\"},
                {\"time\": \"12:00\", \"temp\": \"24.0\", \"icon\": \"☀️\", \"hex\": \"#f9e2af\", \"owm\": \"01d\"},
                {\"time\": \"15:00\", \"temp\": \"26.0\", \"icon\": \"☁️\", \"hex\": \"#bac2de\", \"owm\": \"03d\"},
                {\"time\": \"18:00\", \"temp\": \"23.0\", \"icon\": \"☁️\", \"hex\": \"#bac2de\", \"owm\": \"04d\"},
                {\"time\": \"21:00\", \"temp\": \"20.0\", \"icon\": \"🌙\", \"hex\": \"#cba6f7\", \"owm\": \"01n\"}
            ]
        },"
    done
    final_json="${final_json%,}]"
    ...
    if [ ! -z "$processed_forecast" ]; then
        ...
        for d in $dates; do
            ...
            for i in $(seq 0 1 $count_slots); do
                ...
                hourly_json="${hourly_json} {\"time\": \"${s_time}\", \"temp\": \"${s_temp}\", \"icon\": \"${s_icon}\", \"hex\": \"${s_hex}\", \"owm\": \"${s_code}\"},"
            done
...
elif [[ "$1" == "--current-emoji" ]]; then
    curr_time=$(date +%H:%M)
    owm=$(cat "$json_file" | jq -r --arg ct "$curr_time" '
        .forecast[0] as $day |
        (($day.hourly | map(select(.time <= $ct)) | last) // $day.hourly[0] | .owm) // $day.owm // "02d"
    ')
    [[ -z "$owm" || "$owm" == "null" ]] && owm="02d"
    get_emoji "$owm"
fi
