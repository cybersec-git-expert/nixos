...
Item {
    ...
    function openScheduleLink(url) {
        if (!url || url.length === 0)
            return;
        if (url.startsWith("obsidian://"))
            Quickshell.execDetached(["obsidian", url]);
        else
    ...
    // -------------------------------------------------------------------------
    // Arrow keys: always move the calendar month (predictable). Weather day uses Alt+arrows.
    Shortcut {
        sequence: "Left"
        onActivated: window.setMonthOffset(window.targetMonthOffset - 1)
    }
    Shortcut {
        sequence: "Right"
        onActivated: window.setMonthOffset(window.targetMonthOffset + 1)
    ...
    Shortcut {
        sequence: "Alt+Left"
        onActivated: window.setWeatherView(window.targetWeatherView - 1)
    }
    Shortcut {
        sequence: "Alt+Right"
        onActivated: window.setWeatherView(window.targetWeatherView + 1)
    ...
    Timer {
        ...
        onTriggered: {
            ...
            // Refresh “today” highlight when the local calendar date changes.
            let sig = window.currentTime.getFullYear() + "-" + window.currentTime.getMonth() + "-" + window.currentTime.getDate();
            if (window._calDateSig !== sig) {
                window._calDateSig = sig;
                updateCalendarGrid();
            }
        }
    }
    ...
    property int activeHourIndex: {
        ...
        function slotToMinutes(t) {
            ...
            let mm = parseInt(p[1] || "0", 10);
            if (isNaN(hh)) hh = 0;
            if (isNaN(mm)) mm = 0;
            return hh * 60 + mm;
        }

        let nowM = window.currentTime.getHours() * 60 + window.currentTime.getMinutes();
        let hrArr = window.weatherData.forecast[0].hourly.slice(0, 8);
        let bestIdx = 0;
        let bestDiff = 99999;
        for (let i = 0; i < hrArr.length; i++) {
            let diff = Math.abs(slotToMinutes(hrArr[i].time) - nowM);
            if (diff < bestDiff) {
                bestDiff = diff;
                bestIdx = i;
            }
    ...
    readonly property string scheduleLinkButtonLabel: {
        if (!scheduleData || !scheduleData.link)
            return "Open";
        const u = scheduleData.link;
    ...
    // Emoji + Latin: avoids missing "Iosevka Nerd Font" (PUA tofu boxes).
    readonly property string uiSymFont: "JetBrains Mono, Noto Color Emoji, Noto Emoji, sans-serif"
    ...
    function updateCalendarGrid() {
        ...
        d.setDate(1);
        // Use targetMonthOffset so header + grid match as soon as prev/next is clicked
        // (monthOffset only syncs mid-transition animation).
    ...
    onTargetMonthOffsetChanged: updateCalendarGrid()

    Component.onCompleted: {
        updateCalendarGrid();
    }

    // Wing width + gauge cell size shrink when the popup is narrower than the design (small monitors / clamped shell width) so the right column still fits without clip: true eating the FEELS gauge.
    readonly property real gaugeHorizPad: window.s(40)
    readonly property real gaugeSpacing: window.width > 0 && window.width < window.s(1520) ? window.s(10) : window.s(16)
    readonly property real _maxWingHalf: window.width > 8
        ? Math.max(window.s(220), (window.width - window.s(640)) / 2 - window.s(56))
        : window.s(380)
    readonly property int gaugeCellWidth: {
        const pad = window.gaugeHorizPad;
        const sp = window.gaugeSpacing;
        const raw = Math.floor((window._maxWingHalf - pad - 3 * sp) / 4);
        return Math.max(window.s(52), Math.min(window.s(82), raw));
    }
    readonly property int gaugeRingSize: Math.min(window.s(68), window.gaugeCellWidth - window.s(8))
    readonly property real gaugeRowMinWidth: 4 * window.gaugeCellWidth + 3 * window.gaugeSpacing
    readonly property real wingPanelWidth: Math.min(window._maxWingHalf, Math.max(window.s(280), window.gaugeRowMinWidth + window.gaugeHorizPad))
    ...
    Item {
        ...
        Rectangle {
                        ...
                        delegate: Item {
                            ...
                            Rectangle {
                                ...
                                ColumnLayout {
                                    ...
                                    Text { 
                                        Layout.alignment: Qt.AlignHCenter
                                        text: modelData.icon || (window.weatherData && window.weatherData.forecast[window.weatherView] ? window.weatherData.forecast[window.weatherView].icon : "☁️")
                                        font.family: window.uiSymFont; font.pixelSize: window.s(18)
                ...
                ColumnLayout {
                    ...
                    RowLayout {
                        ...
                        Rectangle {
                            ...
                            Text { anchors.centerIn: parent; text: "📅"; font.family: window.uiSymFont; color: window.text; font.pixelSize: window.s(16) }
                            MouseArea { 
                        ...
                        Rectangle {
                            ...
                            Text { anchors.centerIn: parent; text: "‹"; font.family: "JetBrains Mono"; color: window.text; font.pixelSize: window.s(20) }
                        ...
                        Rectangle {
                            ...
                            Text { anchors.centerIn: parent; text: "+"; font.family: "JetBrains Mono"; color: diaryMa.containsMouse ? window.mauve : window.text; font.pixelSize: window.s(32) }
                            MouseArea { 
            ...
            Item {
                ...
                anchors.rightMargin: window.s(48)
                width: window.wingPanelWidth
                height: window.s(420)
                z: 10 
                ...
                ColumnLayout {
                    ...
                    RowLayout {
                        ...
                        MouseArea { 
                            ...
                            Text { 
                                anchors.centerIn: parent; text: "‹"; font.family: "JetBrains Mono"; font.pixelSize: window.s(20)
                        ...
                        Text {
                            Layout.fillWidth: true
                            Layout.minimumWidth: window.s(80)
                            horizontalAlignment: Text.AlignHCenter
                    ...
                    RowLayout {
                        Layout.fillWidth: true
                        Layout.preferredHeight: window.s(100)
                        Layout.alignment: Qt.AlignHCenter
                        spacing: window.gaugeSpacing
                        ...
                        Repeater {
                            ...
                            Item {
                                ...
                                property string gaugeIcon: index === 0 ? "💨" : index === 1 ? "💧" : index === 2 ? "☔" : "🌡️"
                                ...
                                Rectangle {
                                    ...
                                    width: window.gaugeRingSize
                                    height: window.gaugeRingSize
                                    radius: window.gaugeRingSize / 2
                                    color: window.textAccent
            ...
            Item {
                ...
                Text {
                    ...
                    text: "Schedule strip: add an executable\n~/.config/hypr/scripts/quickshell/calendar/schedule/schedule_manager.sh\nthat prints JSON: { \"header\", \"link\", \"lessons\": [{ \"type\", \"start\", \"end\", \"time\", \"room\", … }] }."
                    font.family: "JetBrains Mono"
                    font.pixelSize: window.s(12)
                    color: window.overlay0
                }
                ...
                ColumnLayout {
                    ...
                    RowLayout {
                        ...
                        Rectangle {
                            ...
                            Text { anchors.centerIn: parent; text: "📅"; font.family: window.uiSymFont; font.pixelSize: window.s(18); color: window.textAccent }
                        }
                        ...
                        Rectangle {
                            width: window.s(132); height: window.s(36); radius: window.s(10)
                            visible: window.scheduleData && window.scheduleData.link && window.scheduleData.link.length > 0
                            ...
                            RowLayout {
                                anchors.centerIn: parent
                                spacing: window.s(6)
                                Text { text: window.scheduleLinkButtonLabel; font.family: "JetBrains Mono"; font.weight: Font.Bold; font.pixelSize: window.s(14); color: schLinkMa.containsMouse ? window.base : window.text }
                                Text { text: "↗"; font.family: window.uiSymFont; font.pixelSize: window.s(14); color: schLinkMa.containsMouse ? window.base : window.text }
                            }
                            
                            MouseArea {
                                id: schLinkMa; anchors.fill: parent; hoverEnabled: true
                                onClicked: if (window.scheduleData && window.scheduleData.link)
                                    window.openScheduleLink(window.scheduleData.link)
                            }
                        }
                    }
                    ...
                    Item {
                        ...
                        Text {
                            text: "No timetable blocks today.\nUse the Obsidian button above for today's daily note (Notes/Daily), or extend schedule_manager.sh with lesson JSON."
                            font.family: "JetBrains Mono"
                            font.italic: true
                            font.pixelSize: window.s(14)
                            color: window.overlay0
                                        ...
                                        Item {
                                            ...
                                            ColumnLayout {
                                                ...
                                                RowLayout {
                                                    ...
                                                    spacing: window.s(8)
                                                    Text { text: "⏱"; font.family: window.uiSymFont; font.pixelSize: window.s(14); color: classNode.isActive ? window.mauve : window.overlay1 }
                                                ...
                                                RowLayout {
                                                    ...
                                                    spacing: window.s(8)
                                                    Text { text: "📍"; font.family: window.uiSymFont; font.pixelSize: window.s(14); color: classNode.isPast ? window.surface2 : window.peach }
