...
Item {
    ...
    ListModel {
        ...
        ListElement { title: "Pomodoro"; target: "timer"; icon: "󱓫"; desc: "Countdown panel (qs_timer_state).\nSuper+T"; preview: "previews/preview_focustime.png" }
    ...
    // Keybinds are loaded dynamically from the live hyprland.conf via
    // watchers/keybinds_fetch.sh so the Guide always reflects reality.
    // The shell script handles $var resolution, `source =` includes, and
    // emits the same { k1, k2, action, cmd } shape this ListModel expects.
    function buildKeybinds() {
        dynamicKeybindsModel.clear();
        keybindsReader.running = false;
        keybindsReader.running = true;
    }

    Process {
        id: keybindsReader
        command: ["bash", "-c", "~/.config/hypr/scripts/quickshell/watchers/keybinds_fetch.sh"]
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                try {
                    ...
                    console.warn("Guide: failed to parse keybinds JSON:", e);
                }
            }
        }
    }

    // Re-read whenever hyprland.conf changes so changes show up without restart.
    Process {
        id: keybindsWatcher
        command: ["bash", "-c",
                  "while [ ! -f ~/.config/hypr/hyprland.conf ]; do sleep 1; done; " +
                  "inotifywait -qq -e modify,close_write,create,move ~/.config/hypr/hyprland.conf"]
        running: true
        onExited: {
            buildKeybinds();
            keybindsWatcher.running = false;
            keybindsWatcher.running = true;
        }
    }
