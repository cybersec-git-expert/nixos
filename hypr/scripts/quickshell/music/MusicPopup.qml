...
Item {
    ...
    // Quickshell often runs with no `$HOME` in the shell — `bash -c '$HOME/...'` becomes `/.config/...` and scripts never run.
    readonly property string _scriptMusicDir: Quickshell.env("HOME") + "/.config/hypr/scripts/quickshell/music"
    readonly property string _musicInfoSh: _scriptMusicDir + "/music_info.sh"
    readonly property string _eqSh: _scriptMusicDir + "/equalizer.sh"
    readonly property string _playerCtlSh: _scriptMusicDir + "/player_control.sh"
    ...
    // Align with TopBar: MPD uses mpc; MPRIS uses playerctl -p (from music_info.sh playerName)
    function runPlayerTransport(verb) {
        var pn = (root.musicData && root.musicData.playerName) ? String(root.musicData.playerName) : "";
        root.execCmd(root._playerCtlSh + " " + verb + " \"\" \"\" \"" + pn + "\"");
    }
    ...
    function applyPresetOptimistically(presetName) {
        ...
        if (presets[presetName]) {
            ...
            execCmd(_eqSh + " preset " + presetName);
        }
    }
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                if (outStr.length > 0) {
                    ...
                    // Empty stdout (script error / PATH) — avoid infinite "Loading..."
    ...
    // EQ panel visibility toggle
    property bool showEq: false
    signal resizeRequested(real newH)

    onShowEqChanged: resizeRequested(root.s(showEq ? 620 : 320))
    Component.onCompleted: resizeRequested(root.s(320))
    ...
    Item {
        ...
        Rectangle {
            ...
            ColumnLayout {
                    ...
                    ColumnLayout {
                            ...
                            Slider {
                                ...
                                onPressedChanged: {
                                    if (pressed) {
                                        ...
                                        root.execCmd(root._playerCtlSh + " seek " + value.toFixed(2) + " " + root.musicData.length + " \"" + safePlayer + "\"");
                        ...
                        RowLayout {
                            ...
                            MouseArea {
                                ...
                                onClicked: root.runPlayerTransport("prev")
                            ...
                            MouseArea {
                                ...
                                onClicked: {
                                    ...
                                    root.musicData = temp;
                                    root.runPlayerTransport("play-pause");
                                }
                            ...
                            MouseArea {
                                width: root.s(30); height: root.s(30)
                                cursorShape: Qt.PointingHandCursor
                                onClicked: root.runPlayerTransport("next")
                                Text { anchors.centerIn: parent; text: ""; color: parent.pressed ? root.text : root.overlay2; font.family: "JetBrainsMono Nerd Font"; font.pixelSize: root.s(24) }
                            }
                            ...
                            MouseArea {
                                ...
                                Text {
                                    anchors.centerIn: parent
                                    text: "󰓻"
                                    font.family: "JetBrainsMono Nerd Font"; font.pixelSize: root.s(22)
                                    color: root.showEq ? root.mauve : (eqToggleMa.containsMouse ? root.text : root.overlay2)
                                    Behavior on color { ColorAnimation { duration: 200 } }
                                    scale: eqToggleMa.containsMouse ? 1.1 : 1.0
                ...
                // ==========================================
                // SEPARATOR + EQUALIZER (collapsible)
                // ==========================================
                Item {
                    id: eqCollapsible
                    Layout.fillWidth: true
                    Layout.preferredHeight: root.showEq ? eqInnerCol.implicitHeight : 0
                    clip: true
                    Behavior on Layout.preferredHeight { NumberAnimation { duration: 350; easing.type: Easing.OutQuint } }
                ...
                ColumnLayout {
                                        ...
                                        Slider {
                                            ...
                                            onPressedChanged: {
                                                if (!pressed) {
                                                    ...
                                                    root.execCmd(root._eqSh + " set_band " + modelData.idx + " " + Math.round(value));
                                                }
                                            }
                    ...
                    } // close eqInnerCol ColumnLayout
                } // close eqCollapsible Item
            }
        }
    }
