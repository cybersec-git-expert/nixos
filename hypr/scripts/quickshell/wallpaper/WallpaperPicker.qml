...
Item {
    ...
    // -------------------------------------------------------------------------
    // OUTPUT TARGET (Monitor 1 / Monitor 2 / Both)
    // -------------------------------------------------------------------------
    ...
    function applyWallpaper(safeFileName, isVideo) {
        ...
        let applyColorsScript = Qt.resolvedUrl("apply_matugen_wallpaper.sh").toString();
        if (applyColorsScript.startsWith("file://")) {
            applyColorsScript = decodeURIComponent(applyColorsScript.substring(7));
        }
        const escApplyColors = escapeBash(applyColorsScript);
        ...
        if (window.currentFilter === "Search" && window.hasSearched) {
            ...
            if (alreadyExists) {
                const applyScript = `
                    (
                        exec > /tmp/qs_wallpaper_apply.log 2>&1
                        echo "=== apply existing ==="
                        echo "DEST_FILE=$DEST_FILE"
                        echo "OUTPUTS_VALUE=$OUTPUTS_VALUE"
        ...
        if (isVideo) {
            ...
            wallpaperCmd = `
                ...
                # GRACEFUL FALLBACK LOOP
                for i in {1..20}; do
                    if env WGPU_BACKEND=vulkan swww img "$WALL_FILE" "\${OUT_ARGS[@]}" --resize crop --transition-type ${randomTransition} --transition-pos 0.5,0.5 --transition-fps 144 --transition-duration 1 >/dev/null 2>&1; then
                        break
                    elif swww img "$WALL_FILE" "\${OUT_ARGS[@]}" --resize crop --transition-type ${randomTransition} --transition-pos 0.5,0.5 --transition-fps 144 --transition-duration 1 >/dev/null 2>&1; then
                        break
                    fi
                    sleep 0.05
                done
        ...
        const fullScript = `
            (
                ...
                ( bash "${escApplyColors}" "$THUMB_FILE" || true ) &
                MATUGEN_PID=$!
    ...
    // NOTE: Keep these as plain filesystem paths (no `file://` prefix).
    // FolderListModel expects a file URL, so we add `file://` at the model sites.
    ...
    Shortcut {
        ...
        onActivated: {
            // If actively typing in the search box, just blur it.
            if (searchInput.activeFocus) {
                searchInput.focus = false;
                view.forceActiveFocus();
                return;
            }
            // If on the Search filter (but not typing), back out to the All
            // filter as a first ESC press.
            if (window.currentFilter === "Search") {
    ...
    FolderListModel {
        id: localFolderModel
        // Render directly from source wallpapers. Some cached thumbs can be in
        // formats Qt can't decode; using srcDir avoids persistent thumbDir warnings.
        folder: "file://" + window.srcDir
        nameFilters: ["*.jpg", "*.jpeg", "*.png", "*.webp", "*.gif", "*.mp4", "*.mkv", "*.mov", "*.webm"]
        showDirs: false
    ...
    ListView {
        ...
        delegate: Item {
            ...
            // Treat common video extensions as video too (when sourcing from srcDir)
            readonly property bool isVideo: safeFileName.startsWith("000_") || /\.(mp4|mkv|mov|webm)$/i.test(safeFileName)
            ...
            // parent can be null during delegate incubation; guard to avoid log spam
            ...
            Item {
                ...
                Image {
                    anchors.fill: parent
                    property string fallbackSource: "file://" + window.srcDir + "/" + window.getCleanName(delegateRoot.safeFileName)
                    // Never try to decode video files via Image (Qt logs "Unsupported image format").
                    source: (!delegateRoot.isVideo && fileUrl !== undefined) ? fileUrl : ""
    ...
    Rectangle {
        ...
        Row {
            ...
            // Output selector (Monitor 1 / Monitor 2 / Both)
            Item {
                id: outputSelector
                height: window.s(44)
                width: window.s(150)
                ...
                Rectangle {
                    ...
                    anchors.fill: parent
                    radius: window.s(10)
                    color: Qt.rgba(_theme.surface2.r, _theme.surface2.g, _theme.surface2.b, 0.35)
                    border.color: Qt.rgba(_theme.surface1.r, _theme.surface1.g, _theme.surface1.b, 0.8)
                    border.width: 1
                    ...
                    Row {
                        ...
                        Text {
                            ...
                            color: Qt.rgba(_theme.text.r, _theme.text.g, _theme.text.b, 0.9)
                        }
                        Text {
                            text: window.selectedOutputsLabel()
                    ...
                    MouseArea {
                        ...
                        enabled: !window.isApplying
                        onClicked: outputSelector.open = !outputSelector.open
                    }
                }

                Rectangle {
                    id: outputMenu
                    visible: outputSelector.open
                    anchors.top: outputBtn.bottom
                    anchors.topMargin: window.s(8)
                    anchors.left: outputBtn.left
                    width: outputBtn.width
                    height: menuCol.implicitHeight + window.s(12)
                    radius: window.s(12)
                    color: Qt.rgba(_theme.mantle.r, _theme.mantle.g, _theme.mantle.b, 0.96)
                    border.color: Qt.rgba(_theme.surface1.r, _theme.surface1.g, _theme.surface1.b, 0.8)
                    border.width: 1
                    clip: true
                    z: 999
                    ...
                    Column {
                        ...
                        Repeater {
                            ...
                            delegate: Rectangle {
                                ...
                                height: window.s(34)
                                radius: window.s(10)
                                color: (outputRowMa.containsMouse || outputPrefs.targetOutputsMode === modelData.mode)
                                       ? Qt.rgba(_theme.surface2.r, _theme.surface2.g, _theme.surface2.b, 0.55)
                                       : "transparent"
                                ...
                                MouseArea {
                                    ...
                                    cursorShape: Qt.PointingHandCursor
                                    onClicked: {
                                        outputPrefs.targetOutputsMode = modelData.mode;
                                        outputSelector.open = false;
                                    }
                                }
                            }
                        }
