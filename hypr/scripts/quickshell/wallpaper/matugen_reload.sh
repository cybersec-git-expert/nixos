...
# Hyprlock: matugen templates cannot express rgba(hex) safely; derive from qs_colors.json.
python3 "$HOME/.config/hypr/scripts/quickshell/wallpaper/render_hyprlock_from_qs_colors.py" 2>/dev/null || true
...
if command -v jq &>/dev/null && command -v hyprctl &>/dev/null && hyprctl version &>/dev/null; then
	_j="$HOME/.config/hypr/scripts/quickshell/qs_colors.json"
	if [[ -f "$_j" ]]; then
		_strip() { echo "${1//#/}"; }
		_blue=$(_strip "$(jq -r '.blue // empty' "$_j")")
		_inact=$(_strip "$(jq -r '.surface2 // .overlay0 // empty' "$_j")")
		_shadow=$(_strip "$(jq -r '.crust // .mantle // empty' "$_j")")
...
# Generate kitty color theme from qs_colors.json (background matches bar base color)
if [[ -f "$QS_JSON" ]]; then
...
# Reload Kitty instances (NixOS may not include killall)
