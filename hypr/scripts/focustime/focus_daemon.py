...
# Live JSON for Quickshell must live in a fixed path: Quickshell often has no
# XDG_RUNTIME_DIR, while this daemon does — writing under /run/user/... broke the UI.
STATE_DIR = os.path.expanduser("~/.cache/focustime")
os.makedirs(STATE_DIR, exist_ok=True)
STATE_FILE = os.path.join(STATE_DIR, "focustime_state.json")

# Hyprland IPC socket (real session runtime only; never use STATE_DIR for this).
HYPR_RUNTIME_DIR = os.environ.get("XDG_RUNTIME_DIR")
...
def migrate_legacy_state_file():
    ...
    if not rd:
        return
    legacy = os.path.join(rd, "focustime_state.json")
    try:
        if os.path.isfile(legacy) and (not os.path.isfile(STATE_FILE) or os.path.getsize(STATE_FILE) == 0):
...
def get_active_window_hyprctl():
    try:
        ...
        if (
            "quickshell" in app_cls.lower()
            or "qs-master" in raw_title.lower()
            or "qs-master" in app_cls.lower()
            or "aylur" in app_cls.lower()
            or "ags" in app_cls.lower()
        ):
...
def listen_hyprland_ipc():
    ...
    sock_path = None
    if HYPR_RUNTIME_DIR:
        cand = f"{HYPR_RUNTIME_DIR}/hypr/{hypr_sig}/.socket2.sock"
